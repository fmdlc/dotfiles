...
function _displayDefault() {
  ...
  # Check if the current directory is a git repository and not the .git directory
    setKey 1 "✅ `kubectl config current-context`"
    setKey 2 "NS: `kubectl config view --minify --output 'jsonpath={..namespace}'`"
    setKey 3 "💀 PODS" "kubectl get pods"
    setKey 4 "💀 SVC" "kubectl get svc"
    setKey 5 "💀 Deployments" "kubectl get deployments"
  ...
  if [[ "$TOUCHBAR_GIT_ENABLED" = true ]] &&
    ...
    setKey 1 "✅ `kubectl config current-context`"
    setKey 2 "NS: `kubectl config view --minify --output 'jsonpath={..namespace}'`"
    setKey 4 "💀 PODS" "kubectl get pods"
    setKey 5 "💀 SVC" "kubectl get svc"
    setKey 6 "💀 Deployments" "kubectl get deployments"
    setKey 3 "🎋 `git_current_branch`" _displayBranches '-q'
    setKey 7 $touchbarIndicators "git status"
    setKey 8 "🔼 push" "git push origin $(git_current_branch)"
    setKey 9 "🔽 pull" "git pull origin $(git_current_branch)"
  fi
}
